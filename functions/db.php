...
{
    $link = mysqli_connect(get_value($config, "host"), get_value($config, 'user'), get_value($config, 'password'),
        get_value($config, 'database'));
...
function insert_lot($link, $lots, $user_id)
{
    ...
    $stmt = db_get_prepare_stmt($link, $sql, [
        get_value($lots, 'title'),
        get_value($lots, 'description'),
        get_value($lots, 'img'),
        get_value($lots, 'category_id'),
        get_value($lots, 'price'),
        get_value($lots, 'end_time'),
        get_value($lots, 'step_rate'),
        $user_id
    ]);
...
function insert_user($link, $user)
{
    ...
    $stmt = db_get_prepare_stmt($link, $sql, [
        ...
        get_value($user, 'name'),
        $password,
        get_value($user, 'contacts'),
        get_value($user, 'avatar')
    ]);
...
function get_search_count_lot($link, $search)
{
    ...
    $result = mysqli_query($link,
        "SELECT COUNT(*) as cnt FROM lots WHERE MATCH(title, description) AGAINST ('$search' IN BOOLEAN MODE) AND winner_id IS NULL;");
...
function get_category_count_lot($link, $category_id)
{
    $category_id = mysqli_real_escape_string($link, $category_id);
    $result      = mysqli_query($link,
        'SELECT COUNT(*) as cnt FROM lots WHERE winner_id IS NULL AND category_id = ' . $category_id);
