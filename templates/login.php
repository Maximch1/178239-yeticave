...
<main>
    ...
    <form class="form container <?= $errors ? "form--invalid" : null; ?>" action="login.php" method="post"> <!-- form--invalid -->
        <h2>Вход</h2>
        <div class="form__item <?= get_value($errors,'email') ? " form__item--invalid" : null; ?>"> <!-- form__item--invalid -->
            <label for="email">E-mail*</label>
            <input id="email" type="text" name="login[email]" placeholder="Введите e-mail" value="<?= get_value($login,'email') ? get_value($login,'email') : null; ?>">
