    ...
    <div class="lot-item__content">
        <div class="lot-item__left">
            <div class="lot-item__image">
                <img src="<?= htmlspecialchars(get_value($lot,'image')); ?>" width="730" height="548" alt="<?= htmlspecialchars(get_value($lot,'name')); ?>">
            </div>
            ...
            <div class="lot-item__state">
                ...
                <div class="lot-item__cost-state">
                    <div class="lot-item__rate">
                        ...
                        <span class="lot-item__cost"><?= format_price(htmlspecialchars(get_value($lot,'max_rate'))); ?></span>
                    </div>
                    <div class="lot-item__min-cost">
                        Мин. ставка <span><?= format_price(htmlspecialchars(get_value($lot,'max_rate') + get_value($lot,'step_rate'))); ?></span>
                    </div>
                </div>
                ...
                <form class="lot-item__form" action="lot.php?id=<?= get_value($lot,'id'); ?>" method="post">
                    <p class="lot-item__form-item form__item <?= !empty($errors) ? "form__item--invalid" : null; ?>">
                        ...
                        <input id="cost" type="text" name="rate" placeholder="<?= htmlspecialchars(get_value($lot,'max_rate') + get_value($lot,'step_rate')); ?>" value="<?= htmlspecialchars($rate) ? htmlspecialchars($rate) : null; ?>">
                    ...
                    <table class="history__list">
                        <?php foreach ($bets as $bet): ?>
                            <tr class="history__item">
                                ...
                                <td class="history__time"><?= get_time_format(get_value($bet,'create_time')) ? get_time_format(get_value($bet,'create_time')) : get_value($bet,'format_create_time'); ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </table>
                </div>
